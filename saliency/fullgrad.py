...
class FullGrad():
    ...
    def checkCompleteness(self):
        ...
        input_grad, bias_grad = self.fullGradientDecompose(
            input, target_class=None)
        ...
        err_string = "Completeness test failed! Raw output = " + \
            str(raw_output.max().item()) + " Full-gradient sum = " + \
            str(fullgradient_sum.item())
        assert isclose(raw_output.max().item(), fullgradient_sum.item(
        ), rel_tol=1e-4), err_string + err_message
    ...
    def fullGradientDecompose(self, image, target_class=None):
        ...
        output_scalar = -1. * \
            F.nll_loss(out, target_class.flatten(), reduction='sum')

        input_gradient, feature_gradients = self.model_ext.getFeatureGrads(
            image, output_scalar)
    ...
    def saliency(self, image, target_class=None):
        ...
        for i in range(len(bias_grad)):
            ...
            if len(bias_grad[i].size()) == len(im_size):
                ...
                gradient = F.interpolate(temp, size=(
                    im_size[2], im_size[3]), mode='bilinear', align_corners=True)
